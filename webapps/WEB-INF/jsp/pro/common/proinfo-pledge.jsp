...
<!-- 质押物信息 -->
    <table class="table table-bordered table-hover table-striped table-top" data-toggle="tablefixed">
    	...
    	<tr>
    		<th width="20">序号</th>
    		<th width="40">质押物名称</th>
    		<th width="40">质押物类型</th>
    		<th width="80">质押物描述</th>
    		<th width="80">附件</th>
    	...
    	<tr>
    		<td>${obj.id }</td>
    		<td>${obj.infoName }</td>
    		<td><c:forEach var="p" items="${plist}" varStatus="status"><c:if test="${p.id==obj.infoCategoryId}">${p.pledgeTypeName }</c:if></c:forEach></td>
    		<td>${obj.infoDesc }</td>
