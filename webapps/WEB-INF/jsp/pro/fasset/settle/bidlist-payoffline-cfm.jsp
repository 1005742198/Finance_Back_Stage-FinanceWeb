...
<table class="table table-condensed table-hover" width="100%">
        <tbody>
            <tr>
                <td>
                    <label class="control-label x120">投资人：</label>
                    <span>${orderApply.orderUserName}</span>
                </td>
                <td>
                    <label class="control-label x120">转让时间：</label>
                    <span><fmt:formatDate value="${orderApply.createTime}"/></span>
                </td>
                <td>
                    <label class="control-label x120">报价金额：</label>
                    <span><fmt:formatNumber value="${orderApply.orderMoney}" pattern="0.00" maxFractionDigits="2"/>元</span>
                </td>
                <td>
                    <label class="control-label x120">保证金金额：</label>
                    <span><fmt:formatNumber value="${orderApply.guaranteeMoney}" pattern="0.00" maxFractionDigits="2"/>元</span>
                </td>
            </tr>
            <tr>
                <td>
                ...
                <td colspan="3">
                    ...
                    <c:choose>
                    	<c:when test="${needPayMoney <= 0 }">
                    		<span>支付完成</span>
                    	</c:when>
                    	<c:otherwise>
                    		<span><fmt:formatNumber value="${needPayMoney}" pattern="0.00" maxFractionDigits="2"/>元</span>
                    	</c:otherwise>
                    </c:choose>
                </td>
            </tr>
...
<form id="payoffline_form" action="<%=request.getContextPath()%>/fasset/proissue/settle/payoffline_pay.do" data-toggle="validate" method="post">
...
function updateOffline(obj){
	...
	$.ajax({
		   type: "POST",
		   async: false,
		   url: "<%=request.getContextPath()%>/fasset/proissue/settle/payoffline_pay.do",
