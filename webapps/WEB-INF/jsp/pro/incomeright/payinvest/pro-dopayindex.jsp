...
<div class="bjui-pageContent" id="pro-dopayindex">
        <div style="margin:0px auto 0; width:100%;">
                ...
                <ul class="nav nav-tabs" role="tablist">
                    <c:choose>
	                	<c:when test="${powerlist.contains('/incomeright/proclaims/payinvest/to-audit.do') && not empty projectMoneyApply && projectMoneyApply.applyStatus == 1 }">
	                	...
	                	<c:when test="${powerlist.contains('/incomeright/proclaims/payinvest/to-apply.do') }">
                ...
                <div class="tab-content">
                   <c:choose>
                    ...
                    <div class="tab-pane fade <c:if test="${search.index == 1}">active in</c:if>" id="payinvest">
                    	<%@ include file="/WEB-INF/jsp/pro/incomeright/payinvest/pro-auditpayinvest.jsp" %>
                    </div>
                    <div class="tab-pane fade bjui-layout <c:if test="${search.index == 2}">active in</c:if>" id="paydetail">
                    	<%@ include file="/WEB-INF/jsp/pro/incomeright/payinvest/pro-payinvestdetail.jsp" %>
                    </div>
                    </c:when>
                    <c:when test="${powerlist.contains('/incomeright/proclaims/payinvest/to-apply.do')}">
                    <div class="tab-pane fade <c:if test="${search.index == 1}">active in</c:if>" id="payinvest">
                    	<%@ include file="/WEB-INF/jsp/pro/incomeright/payinvest/pro-applypayinvest.jsp" %>
                    </div>
                    <div class="tab-pane fade bjui-layout <c:if test="${search.index == 2}">active in</c:if>" id="paydetail">
                    	<%@ include file="/WEB-INF/jsp/pro/incomeright/payinvest/pro-payinvestdetail.jsp" %>
                    </div>
