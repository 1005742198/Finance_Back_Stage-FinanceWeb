		...
		<div class="row top-part">
			<div class="col-md-12">	
				<div class="panel panel-default" style="max-height:200px;">
					<div class="panel-heading">产品情况：</div>						
		...
		<div class="row bottom-part">
			<div class="col-md-12">	
                ...
                <ul class="nav nav-tabs" role="tablist">
	                ...
	                <c:choose>
	                	<c:when test="${powerlist.contains('/incomeright/settle/to-releaseaudit.do') && not empty projectMoneyApply && projectMoneyApply.applyStatus == 1 }">
	                		<li <c:if test="${!project.isCfmrightinfo() }">class="active"</c:if>>
							<a href="<%=request.getContextPath()%>/incomeright/settle/to-releaseaudit.do?projectId=${project.id}" 
							role="tab" data-toggle="ajaxtab" data-target="#releaseMoney"
							data-reload="false">复核放款</a></li>
	                	</c:when>
	                	<c:when test="${powerlist.contains('/incomeright/settle/to-releaseapply.do') }">
	                		<li <c:if test="${!project.isCfmrightinfo() }">class="active"</c:if>>
							<a href="<%=request.getContextPath()%>/incomeright/settle/to-releaseapply.do?projectId=${project.id}" 
							role="tab" data-toggle="ajaxtab" data-target="#releaseMoney"
					...
					<c:if test="${project.isNotRefund() }">
						<li <c:if test="${!project.isCfmrightinfo() && !project.isNotReleaseMoney() }">class="active"</c:if>>
							<a href="<%=request.getContextPath()%>/incomeright/settle/to-pro-backindex.do?projectId=${project.id}"
							role="tab" data-toggle="ajaxtab" data-target="#drawbackMoney"
							data-reload="false">退款</a></li>
						<li><a href="<%=request.getContextPath()%>/incomeright/settle/pro-drawbacklist.do?projectId=${project.id}"
                ...
                <div class="tab-content">
               	 	<c:if test="${project.isCfmrightinfo() }">
	                	 	<div class="tab-pane fade active in" id="cfmrightsummary">
		                    	<%@ include file="/WEB-INF/jsp/pro/incomeright/settle/pro-cfmright-summary.jsp" %>
		                    </div>
		                </c:if>
		                <c:if test="${project.isNotReleaseMoney() && (powerlist.contains('/incomeright/settle/to-releaseapply.do')||powerlist.contains('/incomeright/settle/to-releaseaudit.do')) }">
	                    	<div 
	                    ...
	                    <c:if test="${project.isNotRefund() }">
			                ...
			                <c:if test="${project.isCfmrightinfo() || !project.isNotReleaseMoney() }">class='tab-pane fade bjui-layout'</c:if>
			                 ...
			                 data-url="<%=request.getContextPath()%>/incomeright/settle/to-pro-backindex.do?projectId=${project.id}"
			                 ></div>
