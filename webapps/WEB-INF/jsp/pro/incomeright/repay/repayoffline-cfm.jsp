...
<table class="table table-condensed table-hover" width="100%">
        <tbody>
            <tr>
                <td>
                    <label class="control-label x90">产品编号：</label>
                    <span>${project.projectCode}</span>
                 </td>
                <td>
                    <label class="control-label x90 text-tit pull-left">产品名称：</label>
            ...
            <tr>
                ...
                <td>
					...
					<span><fmt:formatNumber value="${obj.principal-obj.paidPrincipal+obj.interest-obj.paidInterest+obj.overInterest-obj.paidOverInterest}"/>元</span>
                </td>
          ...
          <input type="hidden" name="applyOrderId" value="${obj.id}"/>
       	...
       	<input type="hidden" name="applyOrderType" value="${applyOrderType}"/>
            ...
            <th title="到账金额" width="13%"><input type="text" id="transferMoney[#index#]" name="transferMoney" data-rule="required;checkMoney" size="20" data-rule-checkMoney="checkMoney" class="digitUppercase"></th>
             <th title="付款用途" width="13%"><input type="text" id="remark[#index#]" name="remark"></th>
    			...
    			<tr data-id="${item.id}">
    				...
    				<td data-noedit="true">
    					<c:if test="${item.payStatus != 2}">
    							...
    							<a href="<%=request.getContextPath()%>/incomeright/proclaims/repay/repayoffline_pay.do?offlineId=${item.id}" class="btn btn-red row-del" data-confirm-msg="确认提交吗？" data-nodel="true" data-callback="callback" >确认</a>
    					</c:if>
    				</td>
    			</tr>
    		</c:forEach>
