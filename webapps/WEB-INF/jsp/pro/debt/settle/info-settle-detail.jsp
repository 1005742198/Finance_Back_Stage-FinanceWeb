			...
			<div class="col-md-12">	
                ...
                <ul class="nav nav-tabs" role="tablist">
					...
					<c:if test="${project.isShowReleaseDetail() }">
						...
						<a href="<%=request.getContextPath()%>/debt/proissue/settle/settle-release-detail.do?projectId=${project.id}"
							role="tab" data-toggle="ajaxtab" data-target="#releaseMoney" data-reload="false">放款结果</a></li>
						<li><a href="<%=request.getContextPath()%>/debt/proissue/settle/settle-release-list.do?projectId=${project.id}"
					...
					<c:if test="${project.isShowDrawbackDetail() }">
						<li <c:if test="${!project.isCfmrightinfo() && !project.isShowReleaseDetail() }">class="active"</c:if>>
						<a href="<%=request.getContextPath()%>/debt/proissue/settle/settle-drawback-detail.do?projectId=${project.id}"
							role="tab" data-toggle="ajaxtab" data-target="#drawbackMoney" data-reload="false">退款结果</a></li>
						<li><a href="<%=request.getContextPath()%>/debt/proissue/settle/info-settle-drawback-list.do?projectId=${project.id}"
							role="tab" data-toggle="ajaxtab" data-target="#drawbackDetails" data-reload="false">退款明细</a></li>
					</c:if>
				</ul>
                <!-- Tab panes -->
                <div class="tab-content"  data-auto-height="true">
                	<c:if test="${project.isCfmrightinfo() }">
						<div class="tab-pane fade active in" id="cfmrightResult">
							<%@ include file="/WEB-INF/jsp/pro/debt/info/pro-cfmrightinfo.jsp"%></div>
					...
					<c:if test="${project.isShowReleaseDetail() }">
						<div 
							...
							data-url="<%=request.getContextPath()%>/debt/proissue/settle/settle-release-detail.do?projectId=${project.id}"></div>
						...
						<!--  <div class="tab-pane fade bjui-layout" id="releaseDetails" data-toggle="autoajaxload"
							data-url="<%=request.getContextPath()%>/debt/proissue/settle/settle-release-list.do?projectId=${project.id}"></div>-->
					</c:if>
					<c:if test="${project.isShowDrawbackDetail() }">
						<div 
							...
							data-url="<%=request.getContextPath()%>/debt/proissue/settle/settle-drawback-detail.do?projectId=${project.id}"></div>
						...
						<!--  <div class="tab-pane fade bjui-layout" id="drawbackDetails" data-toggle="autoajaxload"
							data-url="<%=request.getContextPath()%>/debt/proissue/settle/info-settle-drawback-list.do?projectId=${project.id}"></div>-->
					</c:if>
