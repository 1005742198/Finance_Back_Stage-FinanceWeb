...
function afterToImportTradeRecord(json){
	if(json.statusCode != 200)
		$(this).alertmsg("error", json.message);
}
...
<div class="bjui-pageHeader" id="prolist-issue-search">
    <form id="pagerForm" data-toggle="ajaxsearch" action="<%=request.getContextPath()%>/debt/proissue/cfmright/prolist.do" method="post">
...
<div class="bjui-pageContent" id="prolist-issue-list">
    <table data-toggle="tablefixed" data-width="100%" >
    	...
    	<tr>
    		...
    		<th align="center" width="15%">融资方信息</th>
    	...
    	<c:forEach var="obj" items="${list}">
	    	<tr>
	    		...
	    		<td>
		    		<c:if test="${powerlist.contains('/debt/proissue/cfmright/startbuy.do') && obj.isPurchase() }">
			    		<a href="<%=request.getContextPath()%>/debt/proissue/cfmright/startbuy.do?projectId=${obj.id}" 
		    				class="btn btn-blue" data-toggle="doajax" data-mask="true"
		    				data-callback="afterChangeStatus()" data-confirm-msg="确定要启动认购？">立即启动认购</a><br/>
			    	</c:if>
		    		<c:if test="${powerlist.contains('/debt/proissue/cfmright/endbuy.do') && obj.isPurchaseEnd() }">
			    		<a href="<%=request.getContextPath()%>/debt/proissue/cfmright/endbuy.do?projectId=${obj.id}" 
		    				class="btn btn-blue" data-toggle="doajax" data-mask="true"
		    				data-callback="afterChangeStatus()" data-confirm-msg="确定要结束认购？">结束认购</a><br/>
		    		</c:if>
	    			<c:if test="${powerlist.contains('/debt/proissue/cfmright/delayPage.do') && obj.isDelayPage() }">
	    			<a href="<%=request.getContextPath()%>/debt/proissue/cfmright/delayPage.do?projectId=${obj.id}" 
	    			...
	    			<c:if test="${powerlist.contains('/debt/proissue/cfmright/beforecfmright.do') && obj.isDocfmright() }">
		    			<a href="<%=request.getContextPath()%>/debt/proissue/cfmright/beforecfmright.do?id=${obj.id}" 
	    				class="btn btn-blue" data-toggle="dialog" data-width="1000"  
