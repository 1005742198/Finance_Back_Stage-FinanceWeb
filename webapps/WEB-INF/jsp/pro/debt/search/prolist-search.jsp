...
<div class="bjui-pageContent"  id="prolist-search-list">
    <table data-toggle="tablefixed" data-width="100%">
		...
		<td>代码：${list.projectCode}<br />
    		...
    		<td>
			挂牌规模：<fmt:formatNumber value="${list.projectMoney}" pattern="0.00"/>元<br />
			预期年化收益率：<fmt:formatNumber value="${list.investProfit*100}" pattern="0.00"/>% <br />
			项目期限：${list.projectLimit}<c:if test="${list.projectLimitTypeId==1}">天</c:if><c:if test="${list.projectLimitTypeId==2}">月</c:if><c:if test="${list.projectLimitTypeId==3}">年</c:if><br />
