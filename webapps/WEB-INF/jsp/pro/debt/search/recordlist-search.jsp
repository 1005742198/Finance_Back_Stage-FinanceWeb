...
<div class="bjui-pageHeader" id="prolist-search-search">
    <form id="pagerForm" data-toggle="ajaxsearch" action="<%=request.getContextPath()%>/debt/record/search.do" method="post">
...
<div class="bjui-pageContent"  id="prolist-search-list">
    <table data-toggle="tablefixed" data-width="100%">
    	...
    	<tr>
		<td>
			代码：<a href="<%=request.getContextPath()%>/debt/proissue/recordinfo/view.do?id=${list.id}" 
    				data-toggle="dialog" data-width="1000"  data-height="600" data-id="dialog-mask" 
    				data-mask="true" >${list.recordCode}</a><br />
    		<span class="text-tit pull-left">名称：</span><a href="<%=request.getContextPath()%>/debt/proissue/recordinfo/view.do?id=${list.id}" 
    				data-toggle="dialog" data-width="1000"  data-height="600" data-id="dialog-mask" 
    				data-mask="true"  class="text-omit pull-left">${list.recordName}</a>
    		</td>
			<td>
			融资方：<a href="<%=request.getContextPath()%>/user/view.do?id=${list.loanUserId}" 
    				data-toggle="dialog" data-width="800"  data-height="600" data-id="dialog-mask" 
    				data-mask="true" >${list.loanUserName}</a>
    		</td>
    		<td>
			拟募集规模：<fmt:formatNumber value="${list.projectMoney}"/>元<br />
			...
			拟还款方式：<c:forEach var="item" items="${systypeRepayList}"><c:if test="${item.id==list.repayTypeId}">${item.repayTypeName}</c:if></c:forEach><br />
			</td>
    		<td>
