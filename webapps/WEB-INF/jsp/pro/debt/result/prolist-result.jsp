...
<div class="bjui-pageHeader" id="prolist-issue-search">
    <form id="pagerForm" data-toggle="ajaxsearch" action="<%=request.getContextPath()%>/debt/proissue/result/prolist.do" method="post">
    ...
    <table data-toggle="tablefixed" data-width="100%">
    	...
    	<c:forEach var="obj" items="${list}">
	    	<tr>
	    		...
	    		<td>
	    			<c:if test="${obj.isCancelissueCheck() && obj.isIssueSuccessed() && obj.isNotReleaseMoney() && powerlist.contains('/debt/proissue/result/to-pro-cancelissue-check.do')}">
	    				<a href="<%=request.getContextPath()%>/debt/proissue/result/to-pro-cancelissue-check.do?projectId=${obj.id}" 
	    				data-toggle="dialog" data-width="800"  
	    				data-height="400" data-id="dialog-mask" data-mask="true">审核取消发行</a><br/>
	    			</c:if>
	    			<c:if test="${!obj.isCancelissueCheck() && obj.isIssueSuccessed() && obj.isNotReleaseMoney() && powerlist.contains('/debt/proissue/result/to-pro-cancelissue-apply.do')}">
	    				<a href="<%=request.getContextPath()%>/debt/proissue/result/to-pro-cancelissue-apply.do?projectId=${obj.id}" 
	    				data-toggle="dialog" data-width="800"  
	    				data-height="400" data-id="dialog-mask" data-mask="true">申请取消发行</a><br/>
	    			</c:if>
	    			<c:if test="${obj.isCfmrightinfo() }">
	    				<a href="<%=request.getContextPath()%>/debt/proissue/proinfo/cfmrightinfo.do?projectId=${obj.id}" 
