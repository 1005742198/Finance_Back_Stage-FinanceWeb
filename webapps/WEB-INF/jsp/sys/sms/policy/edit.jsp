...
<div class="bjui-pageContent">
	<form id="msgadd" action="<%=request.getContextPath()%>/syssmspolicy/update.do" data-toggle="validate" data-reload-navtab="false" data-callback="afterEditChannelCallBack">
		 	...
		 	<tbody>
					...
					<td colspan="2">
						...
						<c:forEach var="obj" items="${sysSmsTypeList}" varStatus="status">
							<%-- <c:if test="${status.index %2 == 0}"> --%><p><%-- </c:if> --%>
								<input value="${obj.typeId }" <c:if test="${obj.open}"> checked </c:if> onclick="return false;"  type="checkbox" data-label="${obj.typeName }" name="smsTypeCheckbox" id="smsTypeCheckbox.${status.index+1}" data-toggle="icheck"><!-- &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; -->
			    			<%-- <c:if test="${status.index%2 != 0}"> --%></p><%-- </c:if> --%>
			    		</c:forEach>
						</div>
					</td>
				...
				<tr>
					<td colspan="2">
						...
						<label class="control-label" >规则说明：</label> <br/>1、手机号段以英文逗号分开，例如:135,136
					</td>
				</tr>
				<tr>
					<td colspan="2">
						...
						<div class="pull-left" style="width:800px;display:inline;">
							<p>
								<input value="1" <c:if test="${not empty checkAll}">checked</c:if> type="checkbox" data-toggle="icheck" data-label="全选" name="smsChannelCheckAll">
							</p>
							<c:forEach var="obj" items="${sysSmsChannelList}" varStatus="status">
								<%-- <c:if test="${status.index %2 == 0}"> --%><p><%-- </c:if> --%>
									<input value="${obj.id }" <c:if test="${obj.open}"> checked </c:if> type="checkbox" onclick="return false;"  data-label="${obj.channelName }" name="smsChannelCheckbox" id="channel.${status.index+1}" data-toggle="icheck"><!-- &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; -->
				    			<%-- <c:if test="${status.index%2 != 0}"> --%></p><%-- </c:if> --%>
				    		</c:forEach>
						</div>
					</td>
...
function afterEditChannelCallBack(json){
	if(json.statusCode == 200) {
		...
		$(this).alertmsg("error", json.message,{okCall:function(){
			if(json.message == "策略信息不存在，请刷新列表后重试") {
				$(this).dialog("closeCurrent");
				$(this).navtab("refresh");
			}
		}});
