...
<script type="text/javascript">
		...
		function validPhone(element){
			if($("#phoneval").val() == element.value)
				return true;
			return $.post( "${ctx}/memberoperator/validphone.do",
					{"phone":element.value},
					function(data){
					}
				);
		...
		function validIdNumber(element){
			if($("#idNumberval").val() == element.value && $("#idTypeval").val() == $("#idType").val())
				return true;
			return $.post( "${ctx}/memberoperator/valididnumber.do",
		...
		function validEmail(element){
			if($("#emailval").val() == element.value)
				return true;
			return $.post( "${ctx}/memberoperator/validemail.do",
					{"email":element.value},
					function(data){}
				);
		}
		...
		function preUpdate(){
			...
			if(operatorType == $("#operatorType").val()){
				$("#mainForm").attr("data-confirm-msg","确定要提交嘛？");
			}else{
				$("#mainForm").attr("data-confirm-msg","操作员类型改变，需要重新设置角色，确定要提交嘛？");
			}
		}
</script>
<!-- 增加操作员 -->
<div class="bjui-pageContent">
	<form id="mainForm" action="${ctx}/memberoperator/update.do" data-toggle="validate" data-confirm-msg=""  data-callback="updatecallback">
		 	...
		 	<tbody>
				...
				<tr>
					<td align="center" colspan="2">
		    			<button class="btn btn-blue" onclick="preUpdate();">保存</button>
