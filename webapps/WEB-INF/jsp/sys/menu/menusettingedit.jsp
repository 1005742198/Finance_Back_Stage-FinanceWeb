...
function validMenuName(element){
	...
	<form>
		...
		<div class="form-group">
			...
			<c:if test="${sysMenu.menuType == 1}">交易所运营平台菜单</c:if>
			<c:if test="${sysMenu.menuType == 2}">会员运营平台菜单</c:if>
			<c:if test="${sysMenu.menuType == 3}">在线交易平台菜单</c:if>
		</div>
		<div class="form-group">
			...
			<select data-toggle="selectpicker" name="adminType" id="AdminType" data-width="auto" disabled >
				...
				<option value="3" <c:if test="${sysMenu.adminType == 3}">selected="selected"</c:if>>直接融资项目</option>
				<option value="4" <c:if test="${sysMenu.adminType == 4}">selected="selected"</c:if>>金融资产转让</option>
				<option value="5" <c:if test="${sysMenu.adminType == 5}">selected="selected"</c:if>>金融产品发行</option>
				</c:if>
				</c:when>
