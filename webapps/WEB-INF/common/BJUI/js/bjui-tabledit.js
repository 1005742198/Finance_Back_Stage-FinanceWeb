        ...
        var tools = {
                ...
                $trs.each(function(i) {
                    ...
                    $tr.find(':input, :file, a, label, div').each(function() {
                        var $child = $(this),
                            ...
                            dataparam = $child.attr('data-param'),
                            datatagid = $child.attr('data-tagid')
                        ...
                        if (dataparam)  $child.attr('data-param', dataparam.replaceSuffix(i))
                        if (datatagid)  $child.attr('data-tagid', datatagid.replaceSuffix(i))
            ...
            initInput: function($tbody) {
                $tbody.find('> tr > td').each(function() {
                    ...
                    if (!$span.length) {
                        $span = $('<span class="input-hold" style="display:block; padding:0 4px; height:0px;overflow:hidden; font-size:12px; visibility:hidden;"></span>')
                        $(this).append($span)
                    }
            ...
            initTbody: function() {
                        ...
                        if (!$td.data('noedit')) {
                            if ($child.length) {
                                if ($child.is('input:checkbox') || $child.is('input:radio')) {
                                	...
                                	//modified by hj
                                    if($th.hasClass('select-group')){
                                    	var val = eval($td.data('val'));
                                    	$child.each(function(i){
                                			if($(this).data('nextselect')) {
                                				$(this).data('val',val[0][i]);
                                				$(this).next().data('val',val[0][i+1]);
                                				$(this).next().append('<option value="'+ val[0][i+1] +'">' + val[1][i+1] + '</option>')
                                        	}
                                    		$(this).find('option[value="'+val[0][i] +'"]').attr('selected', 'selected')
                                    	})
                                	}else if($th.hasClass('select-other')){//modified by hj
                                    	$child.append('<option value="'+ $td.data('val') +'" selected>' + $td.text() + '</option>')
                                    }else {
                                    	$child.find('option[value="'+ $td.data('val') +'"]').attr('selected', 'selected')
                                    }
                                } else if($th.hasClass('tag-group')){// modified by hj
                                	var val = eval($td.data('val'));
                                	$child.attr('value',val[1]);
                                	$th.find('input[type="hidden"]').val(val[0]);
                                } else if ($pic.length) {
                ...
                $tbody
                    .on('click.bjui.tabledit.readonly', '[data-toggle="doedit"]', function(e) {
                    	...
                    	clickEvent && clickEvent.toFunc()
                        _doEdit($(this).closest('tr'))
                    })
                    .on('click.bjui.tabledit.readonly', '[data-toggle="dosave"]', function(e) {
                        var $tr = $(this).closest('tr'), index = $tr.index(), callback = that.options.callback, _this=this;
                        //modified by hj
                        if($.hasFlash.v >= 9){
                        	var uploadQueue=$tr.find('.uploadify-queue');                        	
                        }else{
                        	var uploadQueue=$tr.find('.queue')
                        }		                         
                        if(uploadQueue.length>0 && uploadQueue.html()!='')return;
                        ...
                        if (that.options.action) {
                            ...
                            $tr
                                ...
                                .isValid(function(v) {
                                    if (v) {
                                        //if (callback) {
                                           // var _callback = callback.toFunc()
                                        //} else {
											//modified by hj
                                        	
											$(_this).addClass('disabled');
											
											if($tr.parents('form').data("validator") != undefined) {
												$tr.parents('form').data("validator").holdSubmit(true);
							            	}
                                            var _callback = function(json) {
                                                if (json[BJUI.keys.statusCode] == BJUI.statusCode.ok) {
                                                    _doRead($tr)
													if(callback){
														callback.toFunc().apply(that, [json])
													}
                                                    var act = $tr.find(".row-del").attr("data-action");
                                                    $tr.find(".row-del").attr("href", act+json.id);
                                                } else {
                ...
                function _doEdit($tr) {
                    $tr.removeClass('readonly').find('> td *').each(function() {
                        ...
                        if (toggle) {
                            ...
                            //限制上传个数 modified by hj
                            if(toggle == 'upload') {
                            	var maxFile = $this.attr("data-max-file");
                            	if (typeof maxFile != 'undefined' && typeof val != 'undefined') {
                        			if(maxFile == val.split("|").length)
                        				$this.remove();
                            	}
                            }
                       }
//								                        if ($this.is(':text') || $this.is('textarea')) {//modefied by hj
//								                            $this.off('keydown.readonly').attr('disabled',false)
//								                            if (readonly) $this.prop('readonly', true)
//								                        }
                        
                        
                        if($this.is('textarea')){//modified by hj
                        	$this.text($this.siblings('span').text())
                            $this.show()
                        	$this.siblings('span').remove()
                        }
                        
                        if($this.is(':text')){//modified by hj
                        	$this.val($this.siblings('span').text())
                            $this.show()
                        	$this.siblings('span').remove()
                        }
                        
                        if($this.is('.bjui-tags')){//modified by hj
                        	$this.find('input').val($this.siblings('span').text())
                        	$this.show()
                        	$this.siblings('span').remove()		                        	
                        }
                        ...
                        //bug修复modified by hj
                        $this.is('.bjui-lookup, .bjui-spinner, .bjui-upload') && $this.show()
                        $this.children().is('.uploadify') && $this.show()
...
//								                            $this.on('keydown.readonly', function(e) { e.preventDefault() }).attr('disabled',true).css('cursor','default')
                        
                        if($this.is('textarea')){//modified by hj
                        	var span='<span class="tabledit-txt">'+$this.html()+'</span>';	                        	
                        	$this.hide()
                        	$this.parent().append(span)		                        	
                        }    
                        if($this.is(':text')){//modified by hj
                        	var span='<span class="tabledit-txt">'+$this.val()+'</span>';	                        	
                        	$this.hide()
                        	$this.parent().append(span)		                        	
                        }
                        if($this.is('.bjui-tags')){//modified by hj
                        	var span='<span class="tabledit-txt">'+$this.find('input').val()+'</span>';	                        	
                        	$this.hide()
                        	$this.parent().append(span)		                        	
                        }
                        
                        //bug修复modified by hj
                        $this.is('.bjui-lookup, .bjui-spinner, .bjui-upload') && $this.hide()
                        $this.children().is('.uploadify') && $this.hide()
                    })
                ...
                $tbody.off(delEvent).on(delEvent, '.row-del', function(e) {
                    ...
                    if ($btnDel.data('confirmMsg')) {
                        $btnDel.alertmsg('confirm', $btnDel.data('confirmMsg'), {okCall: function() { 
							//modified by hj
                        	if($btnDel.data('okcallback')){
                        		$btnDel.attr('data-okcallback').toFunc().call(this,$btnDel);
                        	}
                        	tool.delData($btnDel) }})
                    } else {
                        tool.delData($btnDel)
            ...
            delData: function($btnDel) {
                ...
                var currentTr=$btnDel.closest('tr')
                var swfuploadInp=currentTr.find('.swfupload')
            
            
                var _delRow = function(json) {
                	if(options.nodel == undefined)
                		currentTr.remove()
                	if (swfuploadInp.length > 0) { // modified by hj
		            	swfuploadInp.remove();
		            } 	
                	currentTr
				...
				if ($btnDel.attr('href')=='javascript:;' || $btnDel.attr('href')=='#') {
                    _delRow()
                } else {
    ...
    Tabledit.prototype.init = function() {
        ...
        $tr.find('> th').each(function() {
            ...
            //modified by hj
			var addBtn= $th.data('addbtn')
			var id    = $table.attr('id')
            
            if (title){
				if(addBtn){
					$th.html('<button type="button" class="btn-green" data-toggle="tableditadd" data-target="#'+id+'" data-num="1" data-icon="plus">'+title+'</button>');
				}else{
					$th.html(title);
				}				
			}
    ...
    Tabledit.prototype.add = function($table, num) {
        ...
        for (var i = 0; i < num; i++) {
            ...
            $tr.find('> th').each(function() {
				var attr='';
				if($(this).data('nohandle')){
					attr=' data-nohandle="true"';
				}
				$(this).replaceWith('<td'+attr+'>'+ $(this).html() +'</td>')
			})
