...
//		        	$.when($box.find('.bjui-maskProgress').find('.progress').stop().animate({width:'100%'}, 'fast')).done(function(){
//		        	
//		        		$box.find('.bjui-ajax-mask').remove();
//			        })
        	$box.find('.bjui-maskProgress').find('.progress').stop().animate({width:'100%'}, 'fast');
        	$box.find('.bjui-ajax-mask').remove();
        }
        
        var $more=$box.find('.bjui-moreSearch'),$moresearch=$box.find('[data-toggle="moresearch"]');
        if ($more.is(':visible')) {
    ...
    $(document)
        ...
        .on('bjui.ajaxStop', function(e) {
            ...
            if($.support.leadingWhitespace){
            	...
            	ajaxMask.$pr.find('.progress').animate({width:'100%'}, 'fast')
                ajaxMask.$bg.remove()
                ajaxMask.$pr.remove()
            }            
        })
