...
+function ($) {
    ...
    Upload.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            filter: function(files) {
            	//先过滤文件个数
            	if(options.maxFile) {
            		var allFileNames = that.$element.next(".pic-name").val();
            		var had = 0;
            		if(allFileNames) had = allFileNames.split("|").length;
            		if(files.length > options.maxFile - had) {
            			that.$element.find('> .queue').hide()
            			that.$element.alertmsg('error', '只允许上传'+options.maxFile+"个文件！")
            			return [];
            		}
            	}
            ...
            renderQueueItem: function(file) {
                ...
                if (options.showUploadedSize) {
                    ...
                    $fileSize.find('> .totalsize').html(this.formatFileSize(file.size))//modified by hj
                } else {
    ...
    Upload.prototype.onProgress = function(file, loaded, total) {
        ...
        if(options.breakPoints){//modified by hj
        ...
        if (options.showUploadedPercent) {
            $progress.nextAll('.percent').text(percentText)//modified by hj
        }
    ...
    function Plugin(option) {
		...
		if ($.hasFlash.v >= 9) {
		    this.each(function() {
		        ...
		        if (options.onDialogClose && typeof options.onDialogClose == 'string')
		            options.onDialogClose   = options.onDialogClose.toFunc()
		        //modified by hj      
		        options.onDialogClose=function(queueData){		                	
		            if(options.maxFile) {
		                var allFileNames = $element.next(".pic-name").val();
		                var had = 0;
		                if(allFileNames) had = allFileNames.split("|").length;
		                if(queueData.filesQueued > options.maxFile - had) {
		                	$element.alertmsg('error', '只允许上传'+options.maxFile+"个文件！")	                            
		...
		} else if(window.FileReader) { //没有安装falsh插件
