...
+function ($) {
    ...
    Bjuiajax.prototype.ajaxForm = function(options) {
        ...
        var successFn = function(data, textStatus, jqXHR) {
        	if(data[BJUI.keys.statusCode]==BJUI.statusCode.timeout){//modified by hj
        		$form.alertmsg('info', data[BJUI.keys.message] || FRAG.sessionTimout)
    			BJUI.loadLogin()
        	}else{
        ...
        var _submitFn = function() {
            var op = {loadingmask:options.loadingmask,async:options.async!=undefined?options.async:true, type:$form.attr('method'), url:$form.attr('action'), callback:successFn, error:$.proxy(that.ajaxError, that)}
        ...
        if (options.confirmMsg) {
            $form.alertmsg('confirm', options.confirmMsg, {
            	okCall: _submitFn,
            	cancelCall:function(){//modified by hj
	            	if($form.data("validator") != undefined) {
	            		$form.data("validator").holdSubmit(false);
	            	}		            		
            	}
            })
        } else {
...
//		        }
    	...
    	switch(json[BJUI.keys.statusCode]) {
    		...
    		case BJUI.statusCode.info:
    			if (json[BJUI.keys.message]) $element.alertmsg('info', json[BJUI.keys.message])
    			break;
    ...
    Bjuiajax.prototype.divCallback = function(json, $target) {
        ...
        var options,$form;
        if($.CurrentNavtab){
        	$form=$.CurrentNavtab.find('[data-toggle="ajaxsearch"]');
        	options=$form.data();
        }else if($.CurrentDialog){
        	$form=$.CurrentDialog.find('[data-toggle="ajaxsearch"]')
        	options=$formDialog.data();
        }

        if (json.tabid)
            setTimeout(function() { $form?$form.bjuiajax('doSearch', options):that.$element.navtab('reloadFlag', json.tabid) }, 100)
        if (json.dialogid)
            setTimeout(function() { $form?$form.bjuiajax('doSearch', options):that.$element.dialog('refresh', json.dialogid) }, 100)
        if (json.divid)
        ...
        if (that.options.reloadNavtab)
            setTimeout(function() { $form?$form.bjuiajax('doSearch', options):that.$element.navtab('refresh') }, 100)
        if (json.forward) {
    ...
    Bjuiajax.prototype.dialogCallback = function(json) {
        ...
        if (json.closeCurrent && !json.forward)
            ...
            setTimeout(function() { $form?$form.bjuiajax('doSearch', options):that.$element.dialog('refresh') }, 100)
        if (that.options.reloadNavtab)
            setTimeout(function() { $form?$form.bjuiajax('doSearch', options):that.$element.navtab('refresh') }, 100)
        if (json.forward) {
            var _forward = function() {
    ...
    Bjuiajax.prototype.pageCallback = function(options, target) {
        ...
        if ($target && $target.length) {
            ...
            if (form) {
				...
				that.reloadDiv($target,{type:$target.attr('data-type') || 'POST', url:$target.attr('data-url'),loadingmask:op.loadingmask})
			}
        ...
        var search = function() {
            if ($target && $target.length) {
                ...
                if (options.clearQuery) {
                    ...
                    //modified by hj
                    var inpHidden=$element.find('input[type="hidden"]')
                    inpHidden.each(function(){
    ...
    $(document).on('click.bjui.bjuiajax.data-api', '[data-toggle="reloadsearch"]', function(e) {
        ...
        //modified by hj
        if(!$form.parents('.bjui-pageHeader').parent().hasClass('navtabPage')){
        	$form.parents('.bjui-pageHeader').parent().addClass('bjui-layout')
        }
