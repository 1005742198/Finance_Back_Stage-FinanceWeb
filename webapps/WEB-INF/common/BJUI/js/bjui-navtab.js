...
+function ($) {
    ...
    $(function() {
        var INIT_NAVTAB = function() {
            ...
            setTimeout(function() {
            	var options = $.parseJSON($.cookie("navtaboptions"));
            	options?$(document).navtab(options):$main.navtab('switchTab', 'main');
            }, 100)
    ...
    Navtab.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            closeTab: function(index, openTabid) {
                ...
                //关闭页签时改变cookie
  				currentIndex>0?$.cookie("navtaboptions",JSON.stringify(this.getTabs().eq(currentIndex).data('options'))): $.cookie("navtaboptions",null);
	...
	Navtab.prototype.contextmenu = function($obj) {
	    ...
	    $obj.contextmenu({
	        id: 'navtabCM',
	        bindings: {
	            reload: function(t, m) {//modified by hj
	            	var mReload = m.find('[rel="reload"]')
	            	if(!mReload.hasClass('disabled'))that.refresh(t.data('initOptions').id)
	            },
	        ...
	        ctrSub: function(t, m) {
	            ...
	            if (!t.hasClass('active')) {//modified by hj
    ...
    Navtab.prototype.openTab = function() {
        ...
        if (iOpenIndex >= 0) {
            ...
            $tab.on('click', function(e) {
                ...
                if ($target.hasClass('close'))
                	...
                	$(document).trigger('click');//modified by hj
                    return false
                }else
        ...
        tools.scrollCurrent()
        //modified by hj
        //if(!$.support.leadingWhitespace){//ie 8
        	if(tools.getTabs().length>2){
	        	tools.closeTab(1)
	        }
        //}
    ...
    function Plugin(option) {
        ...
        return this.each(function () {
            ...
            if (typeof property == 'string' && $.isFunction(data[property])) {
            	...
            	//add cookie
            	$.cookie("navtaboptions",JSON.stringify(options));
