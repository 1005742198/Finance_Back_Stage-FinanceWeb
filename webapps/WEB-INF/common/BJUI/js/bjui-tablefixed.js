...
+function ($) {
    ...
    Tablefixed.prototype.init = function() {
        ...
        if (this.$container.parent().hasClass('tab-pane')) width = this.$container.parents('.tab-content').width() - 22
    ...
    Tablefixed.prototype.initHead = function() {
        ...
        $thead.wrap('<div class="fixedtableHeader" style="width:'+ (this.options.newWidth - Tablefixed.SCROLLW) + 'px;overflow:hidden;"><div class="fixedtableThead"><table class="table table-bordered" style="width:'+ (this.options.newWidth - Tablefixed.SCROLLW) + 'px;"></table></div></div>')
    ...
    Tablefixed.prototype.initBody = function() {
        ...
        if (this.options.height) {
            ...
            var resizeH = function() {
                var _height = that.$fixed.parent().height(),
                	_fixedH = that.$fixed.find('.fixedtableHeader').height(),//modified by hj
                	_tipH   = that.$fixed.prev('p.red').length?(that.$fixed.prev('p.red').height()+10):0;//modified by hj
                
                that.$fixed.parent().css('overflow', 'hidden')
                that.$fixed.height(_height)
                    .find('.fixedtableScroller').height(_height - _fixedH -_tipH)//modified by hj
            }
        ...
        $tbody.wrap('<div class="fixedtableScroller"'+ style +' style="width:'+ (this.options.newWidth) +'px;"><div class="fixedtableTbody"><table style="width:'+ (this.options.newWidth - Tablefixed.SCROLLW) +'px;"></table></div></div>')
                        ...
                        $fixed.find('> .resizeProxy')
                            ...
                            .basedrag({
                                ...
                                stop:  function() {
                                    ...
                                    var move    = pleft - mleft - $resizeTh.outerWidth()
                                    var cellNum = tools.getCellNum($resizeTh)
                                    var oldW    = $resizeTh.width()
        ...
        var _resizeGrid = function() {
            $('div.bjui-resizeGrid').each(function() {
                var $this  = $(this), $navtab = $this.closest('.navtabPage'),
                    ...
                    $tip   = $fixed.prev('p.red'),//modified by hj
                    tipH   = $tip.length?($tip.height()+10):0,//modified by hj
                ...
                if (width) {
                    $this.find('.bjui-tablefixed').each(function() {
                        var $fixed = $(this),$fixedtableScroller=$fixed.find('.fixedtableScroller');
                        ...
                        if($fixedtableScroller[0].scrollHeight > $fixedtableScroller[0].clientHeight){
                        	$fixedtableScroller.find('.fixedtableScroller').width(realWidth)
                        }else{
                        	$fixedtableScroller.width(realWidth - Tablefixed.SCROLLW)
                        }
                    })
                }
                ...
                $fixed.height(height)
                    .find('.fixedtableScroller').height(height - fixedH - tipH)//modified by hj
            ...
            var resizeH = function() {
                ...
                that.$fixed.parent().css('overflow', 'hidden')
                that.$fixed.height(_height)
                    .find('.fixedtableScroller').height(_height - _fixedH -_tip)//modified by hj
            }
        }
