    ...
    $(function() {
        ...
        $.validator && $.validator.config({
            ...
            rules: {
                ...
                ,url: [/^((http[s]?|ftp):\/\/)?[^\/\.:]+?\..+\w$/i, '网址格式不正确']//modified by hj
                ,postcode: [/^[1-9]\d{5}$/, '邮政编码格式不正确']
                ,chinese: [/^[\u0391-\uFFE5]+$/, '请输入中文']
                ,username: [/^\w{3,12}$/, '请输入3-12位数字、字母、下划线']
                ,password: [/^(?!\d+$)(?![a-zA-Z]+$)([0-9a-zA-Z]{6,30})$/, '密码由6-30位数字、字母组成']//modified by hj
                //,password: [/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[a-zA-Z0-9]{8,16}$/,'密码由8-16位数字、大小写字母组成']//modified by hj
                ,bankcard: [/^(\d{16}|\d{19})$/,'请输入16或19位数字']//modified by hj
