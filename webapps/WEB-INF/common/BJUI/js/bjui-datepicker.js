 ...
 * -- Modified from dwz.datepicker.js 
...
+function ($) {
    ...
    Datepicker.prototype.TOOLS = function() {
        ...
        var tools = {
            changeTmMenu: function(sltClass) {
                $tm.removeClass('hh').removeClass('mm').removeClass('ss').find('> ul').hide()//modified by hj
                if (sltClass) {
            ...
            getDateWrap: function(date) {
                //modified by hj
                var curMonth=that.options.currentMonth || 0;
                if (!date) date = this.parseDate(that.sDate) || new Date(new Date().setMonth(new Date().getMonth() + curMonth));
            ...
            noHolidays: function(date) {
                if(date.getDay() == 0 || date.getDay() == 6) {
                    if(typeof workweekends != 'undefined') {
                        for(var i=0; i<workweekends.length; i++) {
                            if(workweekends[i].getTime() == date.getTime()) {
                                return true;
                            }
                        }
                    }
    ...
    Datepicker.prototype.init = function() {
        ...
        //modified by hj
        var $cont = this.$element.closest('.bjui-pageContent') || $(window);
        ...
        $clearBtn.off(this.events.click_clear).on(this.events.click_clear, function() {
            that.$element.val('').trigger('validate')//modified by hj
        ...
        $okBtn.off(this.events.click_ok).on(this.events.click_ok, function() {
            ...
            if (tools.hasTime()) {
                date.setHours(Number($hh.val()))
                date.setMinutes(Number($mm.val()))
                date.setSeconds(Number($ss.val()))
            }
            tools.closeCalendar(true)
            that.$element.val(tools.formatDate(date)).focus().trigger('validate')//modified by hj
        ...
        $(document).on('bjui.beforeLoadDialog bjui.resizeGrid', function(e) {
            var $dialog = $(e.target)
    ...
    Datepicker.prototype.create = function(dw, minDate, maxDate) {
        ...
        if (startDay > 0) {
            ...
            for (var t = prevDateWrap.days - startDay + 1; t <= prevDateWrap.days; t++) {
                ...
                //modified by hj
                if(!_ctrClass && options.noHolidays) {
                    _ctrClass = tools.noHolidays(_date) ? '' : ' disabled';
                }
    ...
    function Plugin(option) {
        ...
        return this.each(function () {
            ...
            if (typeof property == 'string' && $.isFunction(data[property])) {
                ...
                data.init()
//                if($.CurrentDialog){
//                  $.CurrentDialog.on('bjui.resizeGrid',function(){
//                      data.tools.closeCalendar(true);
//                  })
//                }
            }
        })
