...
+function ($) {
        ...
        var tools = {
                    ...
                    success  : function(json) {
                        if (json.length != 0) {
                            ...
                            for (var i = 0; i < json.length; i++) {
                                ...
                                tagsLength.push(json[i].label.length)
                            ...
                            $menu
                                .css({'top':that.$element.position().top + (that.$element.outerHeight()), 'left':that.$element.position().left,'width':(Math.max.apply(null, tagsLength)+2.5)+'em'})//modified by hj
                                .fadeIn()
                                .find('> li')
                                    .hover(function() {
            ...
            onAfterCreated: function(item, value) {
            	...
            	if(options.callback) {
            		...
            		$("#"+that.options.tagid).val(value);
                    options.callback.toFunc().apply(that, [value,that.$element.attr("id")]);
            	}
                //20160606 end
    ...
    Tags.prototype.init = function() {
        ...
        if (!$.support.leadingWhitespace) { // for ie8
            $element.on('keyup', $.proxy(this.tools.query, this))//propertychange modified by hj
        } else {
